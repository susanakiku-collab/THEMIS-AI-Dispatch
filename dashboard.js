...
async function runAutoDispatch() {
  ...
  assignments = rebundleLongDistanceDirectionalClusters(assignments, activeItems, selectedVehicles, monthlyMap, { roundTripThreshold: 55 });
...
/* ===== THEMIS v6.2 patch start ===== */
(function(){
  function v62Num(val, fallback = 0) {
    const n = Number(val);
    return Number.isFinite(n) ? n : fallback;
  }

  function v62Clamp(val, min, max) {
    return Math.min(max, Math.max(min, Number(val || 0)));
  }

  function v62Area(areaLike) {
    if (typeof areaLike === 'string') return normalizeAreaLabel(areaLike || '');
    return normalizeAreaLabel(
      areaLike?.destination_area ||
      areaLike?.cluster_area ||
      areaLike?.planned_area ||
      areaLike?.casts?.area ||
      areaLike?.area ||
      ''
    );
  ...
  function v62Distance(rowLike) {
    return v62Num(rowLike?.distance_km ?? rowLike?.casts?.distance_km, 0);
  }

  function v62TravelMinutesFromRow(rowLike) {
    const stored = getStoredTravelMinutes?.(rowLike?.casts?.travel_minutes ?? rowLike?.travel_minutes);
    if (stored > 0) return Math.round(stored);
    const km = v62Distance(rowLike);
    return Math.max(1, Math.round(estimateFallbackTravelMinutes?.(km, v62Area(rowLike)) || 0));
  }

  function v62EstimateRoundTripMinutes(rows) {
    const ordered = typeof sortItemsByNearestRoute === 'function'
      ? sortItemsByNearestRoute((Array.isArray(rows) ? rows : []).filter(Boolean))
      : (Array.isArray(rows) ? rows.filter(Boolean) : []);

    if (!ordered.length) return 0;

    try {
      if (typeof getRowsTravelTimeSummary === 'function') {
        const summary = getRowsTravelTimeSummary(ordered) || {};
        const total = v62Num(summary.totalMinutes, 0);
        if (total > 0) return Math.round(total);
      }
    } catch (e) {
      console.warn('v6.2 getRowsTravelTimeSummary fallback', e);
    }

    const first = v62TravelMinutesFromRow(ordered[0]);
    if (ordered.length <= 1) return Math.max(2, first * 2);

    let outbound = first;
    for (let i = 1; i < ordered.length; i += 1) {
      const prev = ordered[i - 1];
      const cur = ordered[i];
      const prevLat = Number(prev?.casts?.latitude || prev?.latitude || 0);
      const prevLng = Number(prev?.casts?.longitude || prev?.longitude || 0);
      const curLat = Number(cur?.casts?.latitude || cur?.latitude || 0);
      const curLng = Number(cur?.casts?.longitude || cur?.longitude || 0);
      const segmentKm = (prevLat && prevLng && curLat && curLng && typeof calculateDistanceKm === 'function')
        ? Number(calculateDistanceKm(prevLat, prevLng, curLat, curLng) || 0)
        : Math.max(0, Math.abs(v62Distance(cur) - v62Distance(prev)));
      const curKm = Math.max(v62Distance(cur), 1);
      const curMinutes = Math.max(v62TravelMinutesFromRow(cur), 1);
      const speedKmh = Math.max(12, (curKm / curMinutes) * 60);
      outbound += Math.max(1, Math.round((segmentKm / speedKmh) * 60));
    }
    return Math.max(2, outbound * 2);
  }

  function v62BundleCompatibilityScore(targetArea, existingAreas) {
    const area = v62Area(targetArea);
    const rows = Array.isArray(existingAreas) ? existingAreas : [];
    let best = 0;
    for (const raw of rows) {
      const ex = v62Area(raw);
      if (!area || !ex) continue;
      const affinity = v62Num(getAreaAffinityScore?.(area, ex), 0);
      const direction = v62Num(getDirectionAffinityScore?.(area, ex), 0);
      const sameCanonical = (getCanonicalArea?.(area) || '') && (getCanonicalArea?.(area) === getCanonicalArea?.(ex));
      const sameGroup = (getAreaDisplayGroup?.(area) || '') && (getAreaDisplayGroup?.(area) === getAreaDisplayGroup?.(ex));
      let score = affinity * 0.8 + Math.max(direction, 0) * 0.7;
      if (sameCanonical) score += 70;
      else if (sameGroup) score += 40;
      if (direction <= -38) score -= 140;
      best = Math.max(best, Math.round(score));
    }
    return Math.max(0, best);
  }

  function v62ComparePriority(a, b) {
    const ap = Array.isArray(a?.priority) ? a.priority : [];
    const bp = Array.isArray(b?.priority) ? b.priority : [];
    const len = Math.max(ap.length, bp.length);
    for (let i = 0; i < len; i += 1) {
      const diff = v62Num(ap[i], 0) - v62Num(bp[i], 0);
      if (diff !== 0) return diff;
    }
    return v62Num(b?.totalScore, 0) - v62Num(a?.totalScore, 0);
  }

  const _THEMIS_V62_BASE_optimizeAssignments = optimizeAssignments;
  optimizeAssignments = function(items, vehicles, monthlyMap) {
    const workingVehicles = (Array.isArray(vehicles) ? vehicles : []).filter(v => normalizeStatus(v?.status) !== 'maintenance');
    const clusters = typeof buildDispatchClusters === 'function' ? buildDispatchClusters(Array.isArray(items) ? items : []) : [];
    const assignments = [];

    if (!workingVehicles.length || !clusters.length) {
      return typeof _THEMIS_V62_BASE_optimizeAssignments === 'function'
        ? (_THEMIS_V62_BASE_optimizeAssignments.apply(this, arguments) || [])
        : [];
    }

    const usage = new Map();
    const manualLastVehicleId = typeof getManualLastVehicleId === 'function' ? getManualLastVehicleId() : null;

    function getState(vehicleId) {
      const key = Number(vehicleId || 0);
      if (!usage.has(key)) {
        usage.set(key, {
          totalAssigned: 0,
          totalDistance: 0,
    ...
    function getHourLoad(vehicleId, hour) {
      return v62Num(getState(vehicleId).hourLoads.get(Number(hour || 0)), 0);
    }

    function getHourAreas(vehicleId, hour) {
      return [...(getState(vehicleId).hourAreas.get(Number(hour || 0)) || [])];
    }

    function getHourRows(vehicleId, hour) {
      return [...(getState(vehicleId).hourRows.get(Number(hour || 0)) || [])];
    }

    function addHourLoad(vehicleId, hour, rows, area) {
      const state = getState(vehicleId);
      const h = Number(hour || 0);
      const safeRows = (Array.isArray(rows) ? rows : []).filter(Boolean);
      state.totalAssigned += safeRows.length;
      state.totalDistance += safeRows.reduce((sum, row) => sum + v62Distance(row), 0);
      state.hourLoads.set(h, getHourLoad(vehicleId, h) + safeRows.length);
      if (area) {
        const arr = state.hourAreas.get(h) || [];
        state.hourAreas.set(h, [...arr, v62Area(area)]);
      }
      const existingRows = state.hourRows.get(h) || [];
      state.hourRows.set(h, [...existingRows, ...safeRows]);
    }
    ...
    function buildCandidate(vehicle, cluster, candidateRows, isLastRun, isDefaultLastHourCluster) {
      const hour = Number(cluster?.hour || 0);
      const rows = (Array.isArray(candidateRows) ? candidateRows : []).filter(Boolean);
      const count = rows.length;
      if (!count) return null;

      const sameHourLoad = getHourLoad(vehicle.id, hour);
      const seatCapacity = Math.max(1, v62Num(vehicle?.seat_capacity, 4));
      if (sameHourLoad >= seatCapacity) return null;
      if (sameHourLoad + count > seatCapacity) return null;

      const normalizedArea = v62Area(cluster?.area || rows[0]);
      const homeArea = v62Area(vehicle?.home_area || '');
      const existingHourAreas = getHourAreas(vehicle.id, hour);
      const existingHourRows = getHourRows(vehicle.id, hour);
      const monthly = monthlyMap?.get(Number(vehicle.id)) || { totalDistance: 0, workedDays: 0, avgDistance: 0 };
      const addedDistance = rows.reduce((sum, row) => sum + v62Distance(row), 0);
      const projectedWorkedDays = Math.max(v62Num(monthly?.workedDays, 0), 1);
      const projectedAvg = (v62Num(monthly?.totalDistance, 0) + addedDistance) / projectedWorkedDays;
      const routeFlowScore = v62Num(getRouteFlowVehicleScore?.(normalizedArea, existingHourAreas, homeArea), 0);
      const routeContinuityPenalty = v62Num(getRouteContinuityPenalty?.(normalizedArea, existingHourAreas, homeArea), 0);
      const vehicleAreaMatch = v62Num(getVehicleAreaMatchScore?.(vehicle, normalizedArea), 0);
      const directionAffinity = v62Num(getDirectionAffinityScore?.(normalizedArea, homeArea), 0);
      const strictHomeScore = v62Num(getStrictHomeCompatibilityScore?.(normalizedArea, homeArea), 0);
      const hardReverseHome = Boolean(isHardReverseForHome?.(normalizedArea, homeArea));
      const hardReverseMixBlocked = existingHourAreas.length ? Boolean(hasHardReverseMix?.(normalizedArea, existingHourAreas)) : false;
      const areaMixGuardScore = v62Num(getAreaMixGuardScore?.(normalizedArea, existingHourAreas, isLastRun || isDefaultLastHourCluster), 0);
      const bundleCompatibility = v62BundleCompatibilityScore(normalizedArea, existingHourAreas);
      const roundTripMinutes = v62EstimateRoundTripMinutes(rows);
      const longBundleTarget = roundTripMinutes >= 55;
      const oppositeDirection = existingHourAreas.some(area => v62Num(getDirectionAffinityScore?.(normalizedArea, area), 0) <= -38);
      const hardDirectionSplitBlocked = longBundleTarget && oppositeDirection;
      const projectedRows = typeof sortItemsByNearestRoute === 'function'
        ? sortItemsByNearestRoute([...existingHourRows, ...rows])
        : [...existingHourRows, ...rows];

      let projectedRotationMinutes = v62EstimateRoundTripMinutes(projectedRows);
      const driverLastTripChecked = Boolean(isDriverLastTripChecked?.(vehicle.id));
      if (driverLastTripChecked) {
        try {
          const summary = typeof getRowsTravelTimeSummary === 'function' ? (getRowsTravelTimeSummary(projectedRows) || {}) : {};
          const sendOnly = v62Num(summary.sendOnlyMinutes, 0);
          if (sendOnly > 0) projectedRotationMinutes = sendOnly;
        } catch (e) {
          console.warn('v6.2 sendOnly fallback', e);
        }
      }

      const homePriorityWeight = v62Num(getLastTripHomePriorityWeight?.(
        normalizedArea,
        homeArea,
        isLastRun,
        isDefaultLastHourCluster
      ...
      const direction_score = Math.round(v62Clamp(
        vehicleAreaMatch * 1.2 +
        routeFlowScore * 1.05 +
        bundleCompatibility * 1.1 +
        Math.max(directionAffinity, 0) * 0.8 +
        strictHomeScore * 0.55 -
        routeContinuityPenalty * 0.55 -
        areaMixGuardScore * 0.15 -
        (oppositeDirection ? 180 : 0) -
        (hardReverseMixBlocked ? 240 : 0),
        -999,
        999
      ));

      const distance_score = Math.round(v62Clamp(
        280 - projectedAvg * 0.55 - getState(vehicle.id).totalDistance * 0.14 - getState(vehicle.id).totalAssigned * 10,
        -999,
        999
      ));

      const return_time_score = Math.round(v62Clamp(
        260 - projectedRotationMinutes - sameHourLoad * 18 - count * 6,
        -999,
        999
      ));

      const lastTripBonus = (isLastRun || isDefaultLastHourCluster)
        ? (homePriorityWeight * 0.35 + (driverLastTripChecked ? 26 : 0) + (Number(vehicle.id) === Number(manualLastVehicleId) ? 12 : 0))
        : 0;

      const totalScore = Math.round(direction_score * 1.85 + distance_score * 0.85 + return_time_score * 1.15 + lastTripBonus);

      const p1_longClusterLock = longBundleTarget
        ? (sameHourLoad > 0 ? Math.max(0, 260 - bundleCompatibility - routeFlowScore - vehicleAreaMatch * 0.6) : 280)
        : 0;

      const p2_55minBundle = roundTripMinutes >= 55
        ? (sameHourLoad > 0 ? Math.max(0, 220 - bundleCompatibility * 1.15 - routeFlowScore * 0.65) : 220)
        : (sameHourLoad > 0 && bundleCompatibility < 55 ? 24 : 0);

      const p3_oppositeDirection =
        (hardDirectionSplitBlocked ? 12000 : 0) +
        (hardReverseMixBlocked ? 10000 : 0) +
        (oppositeDirection ? 8000 : 0) +
        ((isLastRun || isDefaultLastHourCluster) && hardReverseHome ? 5000 : 0);

      const p4_returnTime = Math.max(0, Math.round(projectedRotationMinutes + sameHourLoad * 14 + Math.max(0, routeContinuityPenalty * 0.35)));

      const p5_distanceBalance = Math.max(0, Math.round(projectedAvg * 8 + getState(vehicle.id).totalDistance * 1.8 + getState(vehicle.id).totalAssigned * 22));

      let p6_lastTripWeak = 0;
      if (isLastRun || isDefaultLastHourCluster) {
        if (hasAnyDriverLastTripChecked?.()) {
          p6_lastTripWeak += driverLastTripChecked ? 0 : 24;
        }
        p6_lastTripWeak += homePriorityWeight > 0 ? Math.max(0, 36 - homePriorityWeight * 0.08) : 42;
        if (Number(vehicle.id) === Number(manualLastVehicleId)) p6_lastTripWeak = Math.max(0, p6_lastTripWeak - 8);
      }

      return {
        vehicle,
        sameHourLoad,
        projectedRotationMinutes,
        roundTripMinutes,
        bundleCompatibility,
        direction_score,
        distance_score,
        return_time_score,
        totalScore,
        priority: [p1_longClusterLock, p2_55minBundle, p3_oppositeDirection, p4_returnTime, p5_distanceBalance, p6_lastTripWeak]
      };
    }

    for (const cluster of clusters) {
      const dateStr = els?.actualDate?.value || todayStr();
      const isLastRun = Boolean(isLastClusterOfTheDay?.(cluster, dateStr));
      const defaultLastHour = typeof getDefaultLastHour === 'function' ? getDefaultLastHour(dateStr) : 5;
      const isDefaultLastHourCluster = Number(cluster?.hour || 0) === Number(defaultLastHour || 5);
      const sortedClusterItems = typeof sortItemsByNearestRoute === 'function'
        ? sortItemsByNearestRoute(cluster?.items || [])
        : (cluster?.items || []);

      const clusterCandidates = workingVehicles
        .map(vehicle => buildCandidate(vehicle, cluster, sortedClusterItems, isLastRun, isDefaultLastHourCluster))
        .filter(Boolean)
        .sort(v62ComparePriority);

      if (clusterCandidates.length) {
        const best = clusterCandidates[0];
        const idleVehicleCount = getIdleVehicleCountForHour(cluster.hour);
        const keepTogether =
          isLastRun ||
          isDefaultLastHourCluster ||
          best.roundTripMinutes >= 55 ||
          (best.sameHourLoad > 0 && best.bundleCompatibility >= 70) ||
          (best.sameHourLoad > 0 && best.direction_score >= 140) ||
          idleVehicleCount <= 1;

        if (keepTogether) {
          sortedClusterItems.forEach(item => {
            assignments.push({
              item_id: item.id,
              actual_hour: cluster.hour,
              vehicle_id: best.vehicle.id,
              driver_name: best.vehicle.driver_name || '',
              distance_km: v62Distance(item)
            });
          });
          addHourLoad(best.vehicle.id, cluster.hour, sortedClusterItems, cluster.area);
          continue;
        }
      }

      const splitItems = sortedClusterItems;
      for (const item of splitItems) {
        const itemCluster = {
          ...cluster,
          count: 1,
          area: v62Area(item),
          totalDistance: v62Distance(item),
          items: [item]
        };

        const itemCandidates = workingVehicles
          .map(vehicle => buildCandidate(vehicle, itemCluster, [item], isLastRun, isDefaultLastHourCluster))
          .filter(Boolean)
          .sort(v62ComparePriority);

        if (!itemCandidates.length) continue;
        ...
        addHourLoad(best.vehicle.id, cluster.hour, [item], itemCluster.area);
      }
    }
